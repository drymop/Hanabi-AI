...
class Model(object):
    ...
    def __init__(self, n_hiddens, n_outputs, n_players=3, hand_size=5, n_ranks=5, n_colors=5):
        ...
        with self.graph.as_default():
            ...
            rnn_inputs = tf.layers.dense(all_inputs, n_hiddens * 2, activation=tf.nn.leaky_relu)

            #--------------------------
            # Recurrent layer

            self.rnn_cell = tf.nn.rnn_cell.LSTMCell(n_hiddens, forget_bias=1.0, activation=tf.nn.leaky_relu)
            ...
            dense = tf.layers.dense(rnn_outputs, n_outputs, activation=tf.nn.leaky_relu)
